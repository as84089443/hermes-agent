...
Treat this skill primarily as a **capture → review → promote discipline**, not as a magical autonomous self-healing runtime.

Its highest-value use is to:

- capture corrections, failures, routing misses, and reusable wins as explicit learnings
- review those learnings at natural task/session boundaries
- promote stable, evidence-backed learnings into the right canonical asset
- use hooks mainly for capture/reminders/detection first, not for aggressive automatic mutation

Important operating correction learned in Hermes use:
- the strongest contribution of this skill is disciplined evidence handling
- the multi-memory framing is useful as an organizing model, but the practical win comes from explicit promotion lanes and governance
- do not start by letting hooks rewrite prompts, code, or operating rules automatically

Use explicit promotion targets:
- durable facts → memory
- reusable procedures → skills
- explainable policy/context → wiki
- active review backlog → status page / candidate queue
- verification improvements → checklists / failure taxonomy / recovery recipes
