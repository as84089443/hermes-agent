...
- Direct import with `--domain` validates against the CURRENT page hostname before importing. If you run `cookie-import-browser chrome --domain .slack.com` while the page is `about:blank` or some other site, it fails with `does not match current page domain`. Navigate to `https://slack.com` first, then import.
- Important Slack-specific finding: successful `.slack.com` cookie import does NOT necessarily recreate a usable logged-in Slack workspace session in the controlled browser. In testing, `.slack.com` cookies imported successfully but visiting `https://<workspace>.slack.com` still landed on the sign-in page. Treat Slack as a special case where cookie import may be insufficient.
- When Slack cookie import succeeds but the controlled browser is still unauthenticated, switch to controlling the user's already logged-in real Chrome session instead of retrying cookie import loops.
