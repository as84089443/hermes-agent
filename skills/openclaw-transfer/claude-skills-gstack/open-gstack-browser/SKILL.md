...
## macOS execution failure fallback

On some macOS systems, the gstack `browse/dist/browse` binary exists but is killed immediately with `Killed: 9` / exit 137 before it can print `--help` or `status`. If `spctl --assess --type execute ~/.claude/skills/gstack/browse/dist/browse` reports `invalid or unsupported format for signature`, treat this as a Gatekeeper / syspolicyd execution block, not a normal browse misconfiguration.

In that case, do not burn time retrying `browse` setup loops. Fall back to Hermes native Chrome CDP attach instead:
...
If `/json/version` returns a `webSocketDebuggerUrl`, Hermes can attach with `/browser connect http://localhost:9222` and will control the real Chrome session, including its logged-in cookies. This is the preferred recovery path when the gstack browse binary itself cannot execute.
